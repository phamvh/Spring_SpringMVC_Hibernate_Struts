...
/**
 *This tells hibernate that the entities of this class are cacheable - used for second-level caching.
 *Check out the hibernate.cfg.xml to see that we have enabled the second-level caching and configured hibernate
 *to use ehcache. ehcache is an external framework for caching. It's lib is downloaded and added to build path.
 */
@Cacheable
/**
 *  This is caching strategies or configuration for how cache is done.
 *  READ_ONLY tells hibernate that our data is not modified/updated. Application will only read it so hibernate will not worry about updating the cache.
 *  Another option is READ_WRITE, which is obviously more expensive, and hibernate will has to update the cache when data changes.
 */
